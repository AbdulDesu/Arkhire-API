const { getContributorModel, getContributorByIDModel, getContributorByParticipatorID, checkContributorModel, createContributorModel, deleteContributorModel } = require('../models/contributor')
...
module.exports = {
    getContributorRoom: async (req, res) => {
      ...
      checkContributor: async (req, res) => {
      ...
      const offset = (page - 1) * limit
        ...
        try {
          const result = await checkContributorModel(projectID , searchKey, searchValue, limit, offset)
          if (result.length) {
            res.status(200).send({
              success: true,
